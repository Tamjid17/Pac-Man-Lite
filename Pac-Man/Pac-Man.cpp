...
int main() {
	srand(time(0));
	RenderWindow window(VideoMode(W * ts, H * ts + 3 * ts), "Pac-Man Lite");

	Texture t;
	t.loadFromFile("Assets/title.png");
	Sprite plat(t);

	Texture yw;
	yw.loadFromFile("Assets/youwin.png");
	Sprite youwin(yw);
	youwin.setPosition(100, 210);

	Texture yl;
	yl.loadFromFile("Assets/youlose.png");
	Sprite youlose(yl);
	...
	Texture sc;
	sc.loadFromFile("Assets/score.png");
	Score score(sc);
